...
const parseLines = lines => {
  ...
  for (let i = 3; i < lines.length - 1; i++) {
    ...
    else if (line.slice(0, 57).trim() === "" && line.slice(59, 70).trim().includes("-"))
...
const updateSpecial = (currentSection, text) => {
  ...
  if (!info.requiresDeptPerm && special.requiresDeptPerm) info.requiresDeptPerm = true
  if (!info.requiresInstPerm && special.requiresInstPerm) info.requiresInstPerm = true
  if (!info.isMajorsOnly && special.isMajorsOnly) info.isMajorsOnly = true

  let comIntensive = special.communicationIntensive
  
  if (!info.isIntensive && comIntensive.isIntensive) info.communicationIntensive.isIntensive = true

  if (!info.communicationIntensive.type.written && comIntensive.type.written) info.communicationIntensive.type.written = true
  if (!info.communicationIntensive.type.spoken && comIntensive.type.spoken) info.communicationIntensive.type.spoken = true
  if (!info.communicationIntensive.type.tech && comIntensive.type.tech) info.communicationIntensive.type.tech = true
  if (!info.communicationIntensive.type.visual && comIntensive.type.visual) info.communicationIntensive.type.visual = true

  if (!info.isServiceLearning && special.isServiceLearning) info.isServiceLearning = true
}
